...
** COMMENT In mixed pitch mode in editor
Taking more stuff from Teco
#+begin_src emacs-lisp
...
(after! mixed-pitch
  ...
  (setq variable-pitch-serif-font (font-spec :family "Alegreya" :size 30))
...
#+RESULTS:
| (lambda (&rest _) (run-at-time nil nil (lambda nil (message %s missing the following fonts: %s (propertize Warning! 'face '(bold warning)) (mapconcat (lambda (font) (propertize font 'face 'font-lock-variable-name-face)) '(Merriweather Alegreya) , )) (sleep-for 0.5)))) | +doom-dashboard-init-h | window-divider-mode | +unicode-init-fonts-h | +popup-mode | +ligatures-init-h |

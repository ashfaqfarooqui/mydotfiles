...
| `theme/` | Not stowed — [Whiskers](https://github.com/catppuccin/whiskers)-based generator for hypr/waybar/ghostty/rofi/hyprlock/swaync/btop/lazygit/fzf/starship/neovim/emacs theming (+ matching wallpaper). Native: `mocha`/`latte`/`frappe`/`macchiato`. Custom (repainted via `--color-overrides`, see `theme/palettes/`): `nord`/`gruvbox`/`dracula`/`tokyonight`/`rosepine`/`everforest`/`kanagawa`/`matte-black`/`osaka-jade`. Neovim and Emacs both load a self-generated colorscheme (`nvim/.config/nvim/colors/theme_generated.lua`, `doom/.doom.d/themes/theme-generated-theme.el`) picked up on next launch. Run `just -f theme/justfile apply <name>`, or `SUPER+CTRL+SHIFT+SPACE` for a rofi picker with color-swatch previews (`theme/swatches/`, rebuild via `just -f theme/justfile swatches`) and a marker on the active theme |

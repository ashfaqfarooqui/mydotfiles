...
#+BEGIN_SRC emacs-lisp
  ...
  ;|| || (defun elfeed-download-video-onlyaudio ()
  ;|| ||   "Download a video using youtube-dl."
  ;|| ||   (interactive)
  ;|| ||   (async-shell-command (format "%s -o \"%s%s\" -f bestaudio %s"
  ;|| ||                                youtube-dl-path
  ;|| ||                                youtube-dl-output-dir
  ;|| ||                                "%(title)s.%(ext)s"
  ;|| ||                                (elfeed-entry-link elfeed-show-entry)))) **\/
...
(global-set-key (kbd "C-c h a") 'hydra-avy/body)
...
(global-set-key (kbd "C-c h i") 'soo-ivy/body)
#+end_src
...
(global-set-key (kbd "C-c h c") 'hydra-mu4e-headers/body)
#+end_src

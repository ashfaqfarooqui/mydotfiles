...
** lsp latex
Experimenting with LSP for latex
#+begin_src emacs-lisp
(add-to-list 'load-path "/home/ashfaqf/.emacs.d/vendor/lsp-latex")
(require 'lsp-latex)
...
#+BEGIN_SRC emacs-lisp
    (setq org-agenda-files (list org-inbox-file org-index-file org-personal-file org-phd-file))
...
** COMMENT org-books
Org-books seems to be a good package but has dependencies on helm and helm-org
#+begin_src emacs-lisp

(add-to-list 'load-path "/home/ashfaqf/.emacs.d/vendor/org-books/")
(require 'org-books)
(setq org-books-file "~/Orgs/Literature/books.org")
...
#+BEGIN_SRC emacs-lisp
  (setq org-capture-templates
        ...
        ("b" "Blog idea"
                ...
                ("d" "Diary" entry (file+olp+datetree "~/Orgs/jrl/journal.org")
...
#+RESULTS:
| doom-modeline-set-selected-window | (lambda (frame) (let ((old-frame (selected-frame)) (old-buffer (current-buffer))) (unwind-protect (progn (select-frame frame (quote norecord)) (load-theme (quote doom-one) t)) (if (frame-live-p old-frame) (progn (select-frame old-frame (quote norecord)))) (if (buffer-live-p old-buffer) (progn (set-buffer old-buffer))))) (setq doom-modeline-icon (display-graphic-p))) | eyebrowse-init | x-dnd-init-frame | (lambda (frame) (with-selected-frame frame (load-theme (quote doom-one) t))) |
